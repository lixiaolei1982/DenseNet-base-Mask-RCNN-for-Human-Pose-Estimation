...
from define_loss_metric import Binary_focal_loss, Categorical_focal_loss
...
def log(text, array=None):
    ...
    if array is not None:
        ...
        text += ("shape: {:20}  min: {:10.5f}  max: {:10.5f}".format(
            str(array.shape),
            array.min() if array.size else 0,#array.min() if array.size else "",
            array.max() if array.size else 0))  #array.max() if array.size else ""))
    print(text)
...
class MobileNetV3_Small(MobileNetBase):
    ...
    def build(self, input_image):
        ...
        C2 = x = self._bottleneck(x, 16, (3, 3), e=16, s=2, squeeze=True, nl='RE')

        x = self._bottleneck(x, 24, (3, 3), e=72, s=2, squeeze=False, nl='RE')

        C3 = x = self._bottleneck(x, 24, (3, 3), e=88, s=1, squeeze=False, nl='RE')
        ...
        C4 =x = self._bottleneck(x, 48, (5, 5), e=144, s=1, squeeze=True, nl='HS')
        ...
        C5 = x = self._conv_block(x, 576, (1, 1), strides=(1, 1), nl='HS')
        '''
...
class MobileNetV3_Large_graph(MobileNetBase):
    ...
    def __init__(self,alpha=1.0, *kwargs):
        ...
        """
        super(MobileNetV3_Large_graph, self).__init__(*kwargs)
    ...
    #def call(self, input_image):
        ...
        C2 = x = self._bottleneck(x, 24, (3, 3), e=72, s=1, squeeze=False, nl='RE')

        x = self._bottleneck(x, 40, (5, 5), e=72, s=2, squeeze=True, nl='RE')

        x = self._bottleneck(x, 40, (5, 5), e=120, s=1, squeeze=True, nl='RE')

        C3 = x = self._bottleneck(x, 40, (5, 5), e=120, s=1, squeeze=True, nl='RE')
        ...
        C4 = x = self._bottleneck(x, 112, (3, 3), e=672, s=1, squeeze=True, nl='HS')
        ...
        C5 = x = self._conv_block(x, 960, (1, 1), strides=(1, 1), nl='HS')
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    loss = K.sparse_categorical_crossentropy(target=anchor_class,
                                             ...
                                             from_logits=True)
    #focal loss       
    #rpn_class_logits = tf.argmax(rpn_class_logits, axis=-1)
    #anchor_class = tf.cast(anchor_class, 'float32')
    #rpn_class_logits = tf.cast(rpn_class_logits, 'float32')
    #loss = Categorical_focal_loss(anchor_class,rpn_class_logits)
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=target_class_ids, logits=pred_class_logits) #sparse labels: no one hot  logits:one hot

    #focal loss
    #pred_class_ids = tf.cast(pred_class_ids, 'float32')
    #target_class_ids = tf.cast(target_class_ids, 'float32')
    #loss = Categorical_focal_loss(target_class_ids, pred_class_ids)
...
def keypoint_mrcnn_mask_loss_graph(target_keypoints, target_keypoint_weights, target_class_ids, pred_keypoints_logit, weight_loss = True, mask_shape=[56,56],number_point=17):
    ...
    #focal loss
    #positive_pred_keypoints_logit = tf.argmax(positive_pred_keypoints_logit, axis=-1)
    #positive_pred_keypoints_logit = tf.cast(positive_pred_keypoints_logit, 'float32')
    #positive_target_keypoints = tf.cast(positive_target_keypoints, 'float32')
    ...
    loss = K.switch(tf.size(positive_target_keypoints) > 0,
                    ...
                    #focal loss
                    #lambda: Categorical_focal_loss(positive_target_keypoints, positive_pred_keypoints_logit),
                    lambda: tf.constant(0.0))
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        #_, C2, C3, C4, C5 = MobileNetV3_Large_graph().build(input_image)
